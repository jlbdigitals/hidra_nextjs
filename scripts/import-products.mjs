...
async function main() {
  const productsPath = join(process.cwd(), "public", "products.json");
  let raw;
  try {
    raw = JSON.parse(readFileSync(productsPath, "utf-8"));
  } catch (err) {
    console.error("No se pudo leer public/products.json:", err.message);
    process.exit(1);
  }
  ...
  if (parseInt(count) > 0) {
    console.log(`La base de datos ya tiene ${count} productos. Saltando importación.`);
    await sql.end();
    return;
  }
